...
fn test_public_key_round_trip() {
    ...
    // Convert to PEM
    let pem_bytes = public_key_to_pem_bytes(&original_public).expect("Failed to convert to PEM");
...
fn test_many_round_trips() {
    for _ in 0..100 {
        ...
        let public_pem = public_key_to_pem_bytes(&public_key).expect("Failed to encode public key");
        let parsed_public =
