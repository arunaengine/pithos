use anyhow::Result;
use pithos_lib::io::pithosreader::PithosReaderError;
use std::fs::File;
use std::io::Read;
use std::path::PathBuf;
use x25519_dalek::{PublicKey, StaticSecret};

pub fn load_private_key_from_pem(filepath: &PathBuf) -> Result<StaticSecret, PithosReaderError> {
...
pub fn load_public_key_from_pem(filepath: &PathBuf) -> Result<PublicKey, PithosReaderError> {
    // Open file handle and read file bytes
    let mut file = File::open(filepath)?;
    let mut file_content = vec![0u8; file.metadata()?.len() as usize];
    file.read_exact(&mut file_content)?;
